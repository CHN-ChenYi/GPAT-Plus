...
def parse_log_baseline_file(log_file):
    found_running = False
    benchmark = ""
    runtime_map = {}
    ...
    with open(log_file, 'r') as f:
        for line in f:
            ...
            if "Running" in line:
                ...
                if benchmark not in runtime_map:
                    runtime_map[benchmark] = []
                    runtime_map[benchmark].append(float(0.0))
                else:
                    runtime_map[benchmark].append(float(0.0))
                continue
            if found_running:
                runtime = float(line.strip())*1000
                runtime_map[benchmark][-1] += runtime
                # add 4 times counter here
                count += 1
                if count == 4:
                    count = 0
...
if __name__ == "__main__":
    ...
    if tiling_method == "AffineTiling":
        ...
        polly_mean, polly_conf, _ = parse_log_baseline_file(polly_log)
        polygeist_mean, polygeist_conf, _ = parse_log_baseline_file(polygeist_log)
